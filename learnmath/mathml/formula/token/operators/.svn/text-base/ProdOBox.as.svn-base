...
class learnmath.mathml.formula.token.operators.ProdOBox extends OBox{
	...
	public function	ProdOBox(parentBox:Box){
		super(parentBox);
	}
	...
	public function draw(graph:MovieClip){
		...
		graph.moveTo(finalBounds.x+w, finalBounds.y + 2*d);
		graph.lineTo(finalBounds.x, finalBounds.y + 2*d);
		...
		graph.lineStyle(s, getHexColor(), 100);
		graph.moveTo(finalBounds.x+ w*z-d, finalBounds.y + h - d);
		graph.lineTo(finalBounds.x+ w*z+d, finalBounds.y + h - d );
		...
		graph.lineStyle(s, getHexColor(), 100);
		graph.moveTo(finalBounds.x + w*(1-z) -d, finalBounds.y + h - d);
		graph.lineTo(finalBounds.x + w*(1-z) +d, finalBounds.y + h - d );
	}
