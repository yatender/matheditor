...
class learnmath.mathml.formula.token.operators.arrows.UParanthesisOBox extends OBox{

	var k:Number = 1/10;
	var l:Number = 1/10;
	var wl:Number = 1;

	public function	UParanthesisOBox(parentBox:Box){
		super(parentBox);
	}
	...
	public function draw(graph:MovieClip){
		...
		graph.moveTo(finalBounds.x, finalBounds.y + finalBounds.height*0.1);
		graph.curveTo(finalBounds.x+finalBounds.width/2, finalBounds.y + finalBounds.height*0.5, finalBounds.x+finalBounds.width, finalBounds.y+ finalBounds.height*0.1);
	}
