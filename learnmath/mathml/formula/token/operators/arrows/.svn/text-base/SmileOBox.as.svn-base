...
class learnmath.mathml.formula.token.operators.arrows.SmileOBox extends OBox{

	var k:Number = 1/10;
	var l:Number = 1/10;
	var wl:Number = 1;

	public function	SmileOBox(parentBox:Box){
		super(parentBox);
	}
	...
	public function draw(graph:MovieClip){
		...
		graph.curveTo(finalBounds.x+finalBounds.width/2, finalBounds.y + finalBounds.height, finalBounds.x+finalBounds.width, finalBounds.y+ finalBounds.height*0.6);
	}
