...
class learnmath.mathml.formula.token.operators.arrows.HatOBox extends OBox{

	var k:Number = 1/10;
	var l:Number = 1/10;
	var wl:Number = 1;

	public function	HatOBox(parentBox:Box){
		super(parentBox);
	}
	...
	public function draw(graph:MovieClip){
		...
		graph.moveTo(finalBounds.x+wl, finalBounds.y + finalBounds.height*3/4);
		graph.lineTo(finalBounds.x+finalBounds.width/2, finalBounds.y + finalBounds.height*0.4);
		graph.lineTo(finalBounds.x+finalBounds.width-wl, finalBounds.y + finalBounds.height*3/4);
	}
