...
class learnmath.mathml.formula.token.operators.arrows.OBracketOBox extends OBox{

	var k:Number = 1/10;
	var l:Number = 1/10;
	var wl:Number = 1;

	public function	OBracketOBox(parentBox:Box){
		super(parentBox);
	}
	...
	public function draw(graph:MovieClip){
		...
		graph.moveTo(finalBounds.x + wl, finalBounds.y + finalBounds.height*0.8);
		graph.lineTo(finalBounds.x + wl, finalBounds.y + finalBounds.height*0.5);
		graph.lineTo(finalBounds.x+finalBounds.width-wl, finalBounds.y + finalBounds.height*0.5);
		graph.lineTo(finalBounds.x+finalBounds.width-wl, finalBounds.y + finalBounds.height*0.8);
